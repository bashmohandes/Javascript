(() => {
    ...
    const api = async (url, options = {}) => {
        ...
        if (!response.ok) { const error = new Error(body.error || 'Request failed.'); Object.assign(error, { status: response.status, code: body.code, current: body.current }); throw error; }
        return body;
    };
    ...
    const requestAuthentication = intro => {
        if (currentUser) return Promise.resolve(currentUser);
        dialog.querySelector('.arcade-auth-intro').textContent = intro || 'Use one gamertag to save play history and scores across every game.';
        dialog.returnValue = ''; if (!dialog.open) dialog.showModal();
        return new Promise(resolve => authenticationWaiters.push(resolve));
    };
    ...
    const render = () => {
        ...
        if (currentUser) {
            ...
            const signIn = document.createElement('button'); signIn.type = 'button'; signIn.textContent = 'Sign in'; signIn.addEventListener('click', () => requestAuthentication()); account.append(signIn);
        }
        applyTheme();
        publish('account:user-changed', { user: currentUser });
    };
    dialog.addEventListener('close', async () => {
        if (!['login', 'register'].includes(dialog.returnValue)) { settleAuthentication(null); return; }
        ...
        try {
            ...
            currentUser = result.user; form.reset(); message.textContent = ''; render(); settleAuthentication(currentUser);
