...
21. [Share online room identity and message contracts](0021-shared-online-room-contracts.md) — **Accepted**; common identity primitives and stable WebSocket envelopes have one implementation while game-specific room managers retain authority and viewer-safe serialization.
22. [Share deterministic Tic-tac-toe mechanics](0022-shared-tictactoe-mechanics.md) — **Accepted**; one immutable transition and minimax core powers browser play and authoritative rooms without moving online trust to the client.
23. [Separate testable Sudoku mechanics](0023-testable-sudoku-mechanics.md) — **Accepted**; modern grid generation, validation, peer notes, and stepwise solving live in a deterministic core while the browser retains presentation and results.
24. [Isolate Pong online client lifecycle](0024-pong-online-client-lifecycle.md) — **Accepted**; a tested browser adapter owns sockets, opaque resume credentials, reconnects, coalesced input, and per-room snapshot sequencing while the server remains authoritative.
25. [Use a code-native pixel identity for the modern arcade](0025-pixel-arcade-identity.md) — **Accepted**; the compatible `playful` id becomes the Pixel default while modern games share crisp local typography, code-native graphics, scoped themes, and accessible motion.
26. [Separate alpha builds from controlled stable releases](0026-controlled-release-trains.md) — **Accepted**; `master` publishes opt-in alpha images while manually approved SemVer releases are built from a protected `release` branch and share curated notes with the product.
27. [Use versioned private cloud game saves](0027-versioned-cloud-game-saves.md) — **Accepted**; signed-in local games use bounded SQLite slots and game-owned importers while online rooms and result trust remain separate.
28. [Bound cloud-save request and storage resources](0028-cloud-save-resource-budgets.md) — **Accepted**; incremental body accounting and transactional account/global budgets prevent authenticated save traffic from starving the event loop or exhausting persistent storage.
