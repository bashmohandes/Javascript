...
test('reuses synchronized arena geometry until an authoritative impact changes it', () => { const { rooms, host, guest } = started(); rooms.broadcastState(host.room); rooms.broadcastState(host.room); for (const player of [host.player, guest.player]) assert.equal('arena' in player.socket.messages.at(-1).state, false); host.room.game.impactSerial += 1; rooms.broadcastState(host.room); for (const player of [host.player, guest.player]) assert.deepEqual(player.socket.messages.at(-1).state.arena, core.snapshot(host.room.game).arena); });
