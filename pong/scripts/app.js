...
const game = {
    ...
    lastTouch: 0, powerUps: [], effects: [{}, {}], touchTargets: [null, null]
};
const paddles = [
...
function newGame() {
    game.score = [0, 0]; game.over = false; game.elapsed = 0; game.lastTouch = 0; game.touchTargets = [null, null]; setScore(0, 0); setScore(1, 0);
...
function update(dt) {
    ...
    paddles[0].vy = game.touchTargets[0] === null ? (game.keys.has('KeyS') ? leftSpeed : 0) - (game.keys.has('KeyW') ? leftSpeed : 0) : Math.max(-leftSpeed, Math.min(leftSpeed, (game.touchTargets[0] - paddles[0].y - paddles[0].h / 2) * 12));
    if (game.mode === 'duo') { const rightSpeed = (game.effects[1].quickUntil || 0) > game.elapsed ? 625 : 500; paddles[1].vy = game.touchTargets[1] === null ? (game.keys.has('ArrowDown') ? rightSpeed : 0) - (game.keys.has('ArrowUp') ? rightSpeed : 0) : Math.max(-rightSpeed, Math.min(rightSpeed, (game.touchTargets[1] - paddles[1].y - paddles[1].h / 2) * 12)); }
...
const pitchPointers = new Map();
function movePaddleWithPointer(event) { const side = pitchPointers.get(event.pointerId); if (side === undefined) return; const bounds = canvas.getBoundingClientRect(); game.touchTargets[side] = (event.clientY - bounds.top) * game.height / bounds.height; }
canvas.addEventListener('pointerdown', event => { if (event.pointerType === 'mouse') return; const bounds = canvas.getBoundingClientRect(); const side = event.clientX - bounds.left < bounds.width / 2 ? 0 : 1; if (side === 1 && game.mode !== 'duo') return; event.preventDefault(); canvas.setPointerCapture(event.pointerId); pitchPointers.set(event.pointerId, side); movePaddleWithPointer(event); });
canvas.addEventListener('pointermove', movePaddleWithPointer);
function releasePitchPointer(event) { const side = pitchPointers.get(event.pointerId); if (side === undefined) return; pitchPointers.delete(event.pointerId); if (![...pitchPointers.values()].includes(side)) game.touchTargets[side] = null; }
canvas.addEventListener('pointerup', releasePitchPointer); canvas.addEventListener('pointercancel', releasePitchPointer);
