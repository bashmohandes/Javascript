...
let arenaColors = {};
let serveTimer = null, serveDueAt = 0, pendingServeMs = null;
...
function clearServeTimer() { clearTimeout(serveTimer); serveTimer = null; serveDueAt = 0; }
function scheduleServe(delay) { clearServeTimer(); pendingServeMs = Math.max(0, delay); serveDueAt = performance.now() + pendingServeMs; serveTimer = setTimeout(() => { serveTimer = null; serveDueAt = 0; pendingServeMs = null; if (game.running && !game.paused && balls[0].vx === 0) launch(); }, pendingServeMs); }
function pauseServe() { if (!serveTimer) return; pendingServeMs = Math.max(0, serveDueAt - performance.now()); clearServeTimer(); }
function resumeServe() { if (game.running && !game.over && balls[0].vx === 0) scheduleServe(pendingServeMs ?? 0); }
function launch() { clearServeTimer(); pendingServeMs = null; const angle = (Math.random() * .8) - .4; balls[0].vx = game.serve * 410 * Math.cos(angle); balls[0].vy = 410 * Math.sin(angle); game.serve *= -1; events.emit('pong:served', { angle }); }
...
function newGame() {
    clearServeTimer(); pendingServeMs = null;
    saves?.startRun();
...
function togglePause() { if (game.mode === 'online') return; if (game.over || !game.running) { newGame(); return; } game.paused = !game.paused; events.emit('game:paused', { paused: game.paused }); if (game.paused) { pauseServe(); showOverlay('Game paused', 'Click or press Space to continue'); } else { resumeServe(); overlay.hidden = true; } }
function point(side) {
...
saves?.registerAdapter({
    ...
    pause: () => { const wasPaused = game.paused; if (game.mode !== 'online' && game.running) { pauseServe(); game.paused = true; showOverlay('Game paused', 'Close saves to continue'); } return wasPaused; },
    resume: wasPaused => { if (!wasPaused && game.mode !== 'online' && game.running && !game.over) { game.paused = false; resumeServe(); overlay.hidden = true; game.last = performance.now(); } },
    capture: async () => ({ mode: game.mode, elapsedSeconds: Math.max(0, Math.round(game.elapsed)), scoreLabel: `${game.score[0]}–${game.score[1]}`, state: { mode: game.mode, running: game.running, paused: game.paused, serve: game.serve, serveDelay: pendingServeMs, score: [...game.score], nextPowerUp: game.nextPowerUp, elapsed: game.elapsed, lastTouch: game.lastTouch, powerUps: game.powerUps.map(item=>({...item})), effects: game.effects.map(item=>({...item})), paddles: paddles.map(item=>({...item,vy:0})), balls: balls.map(item=>({...item})) } }),
    restore: async state => {
