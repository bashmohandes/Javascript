(function (root, factory) {
    ...
    function activatePowerUp(state, player, itemId, authoritativeRandom = () => matchRandom(state)) {
        ...
        inventory.splice(index, 1); const tank = state.tanks[player], generatedIndex = (state.acquiredValues?.[player] || []).findIndex(value => value.powerUpType === itemId), generated = generatedIndex < 0 ? null : state.acquiredValues[player].splice(generatedIndex, 1)[0];
        if (item.kind === 'weapon') {
