(function (root, factory) {
    ...
    function restoreSnapshot(input) {
        ...
        const validGeneratedValue = value => { if (!value || typeof value !== 'object' || Array.isArray(value) || Object.keys(value).some(key => !['powerUpType','capacity','durationTurns','ammunition'].includes(key))) return false; const item = POWER_UP_CATALOG[value.powerUpType]; if (!item) return false; if (item.capacityRange ? !Number.isSafeInteger(value.capacity) || value.capacity < item.capacityRange.min || value.capacity > item.capacityRange.max : value.capacity !== undefined) return false; const duration = item.durationRange || (item.durationTurns ? { min: item.durationTurns, max: item.durationTurns } : null); if (duration ? !Number.isSafeInteger(value.durationTurns) || value.durationTurns < duration.min || value.durationTurns > duration.max : value.durationTurns !== undefined) return false; return item.kind === 'weapon' ? value.ammunition === item.weapon.ammo.perPickup : value.ammunition === undefined; };
        if (!Array.isArray(input.inventories) || input.inventories.length !== 2 || input.inventories.some(items => !Array.isArray(items) || items.length > INVENTORY_LIMIT || items.some(id => !POWER_UP_CATALOG[id])) || input.acquiredValues !== undefined && (!Array.isArray(input.acquiredValues) || input.acquiredValues.length !== 2 || input.acquiredValues.some((values, side) => !Array.isArray(values) || values.length !== input.inventories[side].length || values.some(value => !validGeneratedValue(value)) || values.some((value, index) => value.powerUpType !== input.inventories[side][index]))) || !Array.isArray(input.activeEffects) || input.activeEffects.length !== 2 || !Array.isArray(input.statistics?.powerUpTypesUsed) || input.statistics.powerUpTypesUsed.length !== 2) throw new Error('Invalid Battle Tanks save state.');
        const restored = createInitialState(input.arena.seed), clone = value => value == null ? value : JSON.parse(JSON.stringify(value)), saved = clone(input);
        if (saved.acquiredValues === undefined) saved.acquiredValues = saved.inventories.map((items, side) => { const events = (saved.acquisitionEvents || []).filter(event => event?.player === side); const values = Array(items.length); for (let index = items.length - 1; index >= 0; index -= 1) { const powerUpType = items[index], item = POWER_UP_CATALOG[powerUpType]; let eventIndex = events.length - 1; while (eventIndex >= 0 && events[eventIndex]?.powerUpType !== powerUpType) eventIndex -= 1; const generated = eventIndex >= 0 ? events.splice(eventIndex, 1)[0].generatedValues || {} : {}, value = { powerUpType }; if (item.capacityRange) value.capacity = Number.isSafeInteger(generated.capacity) && generated.capacity >= item.capacityRange.min && generated.capacity <= item.capacityRange.max ? generated.capacity : item.capacityRange.min; const duration = item.durationRange || (item.durationTurns ? { min: item.durationTurns, max: item.durationTurns } : null); if (duration) value.durationTurns = Number.isSafeInteger(generated.durationTurns) && generated.durationTurns >= duration.min && generated.durationTurns <= duration.max ? generated.durationTurns : duration.min; if (item.kind === 'weapon') value.ammunition = item.weapon.ammo.perPickup; values[index] = value; } return values; });
        Object.assign(restored, saved, { onlineMode: false, resultSubmitted: false, startedAt: Date.now() });
