(function (root, factory) {
    ...
    function collectPickup(state, player) {
        ...
        if (collected.length) { if (state.statistics?.powerUps) state.statistics.powerUps[player] += collected.length; state.announcement = `Player ${player + 1} collected ${POWER_UP_CATALOG[collected[0]].label}.`; }
        return collected;
    }
    ...
    function activatePowerUp(state, player, itemId, authoritativeRandom = () => matchRandom(state)) {
        ...
        else if (item.effect === 'heal') { const before = tank.health; tank.health = Math.min(STARTING_HEALTH, tank.health + item.amount); if (state.statistics?.healing) state.statistics.healing[player] += tank.health - before; }
